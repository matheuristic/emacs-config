...
using MacPorts (shown below) or Homebrew or conda. On Debian, install
the ~cmake~ and ~libtool-bin~ APT packages.
        ...
        (let ((word (thing-at-point 'word t)))
          (julia-repl--send-string
           (concat "using REPL;"
                   ...
                   word
                   "\","
                   (number-to-string (length word))
                   ")[1];"
                   "println("
                   "c isa REPL.REPLCompletions.KeywordCompletion"
                   " ? c.keyword : c.mod"
                   ");"
                   "end"
                   )))
