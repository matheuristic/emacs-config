;;; init-org-tufte.el --- Org export engine Tufte-LaTeX backend -*- lexical-binding: t -*-

;; Author: matheuristic

;;; Commentary:

;; Set up minor mode for Org files meant for exporting to Tufte-LaTeX
;; Call `org-tufte-minor-mode' before exporting or add the following
...
;; Requires ox-extra and ox-tufte-latex Emacs packages; download or
;; clone ox-tufte-latex from https://github.com/tsdye/tufte-org-mode
...
and if so returns a modified list with the tufte-ebib cite command."
  ...
  (defun org-tufte-ebib-export (path desc format)
    "Export an ebib link.
See `org-link-parameters' for details about PATH, DESC and FORMAT."
  ...
  (defun org-tufte-minor-mode--setup ()
    ...
    (setq-local
      my-org-tufte-latex-class-base-packages
    ...
    (add-to-list
     ...
     `("tufte-handout"
       ...
       ("\\subsection{%s}" . "\\subsection*{%s}")
       ("\\paragraph{%s}" . "\\paragraph*{%s}")))
    (kill-local-variable 'my-org-tufte-latex-class-base-packages)
  ...
  (defun org-tufte-minor-mode--teardown ()
  ...
  (define-minor-mode org-tufte-minor-mode
    ...
    nil
    ;; startup code
    (if (symbol-value org-tufte-minor-mode)
        (org-tufte-minor-mode--setup)
      (org-tufte-minor-mode--teardown)))
...
(provide 'init-org-tufte)

;;; init-org-tufte.el ends here
